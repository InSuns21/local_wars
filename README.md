...
2. 攻撃・占領・待機を実行
3. 工場や空港でユニットを生産
4. 補給車や空中補給機で燃料・弾薬を立て直す
5. ドローン戦を有効にした場合は工場から自爆ドローンを展開し、対ドローン防空車の迎撃に注意する
6. ターン終了で相手手番へ
...
- 索敵の有無
- 初期資金
- 1ターン収入（都市 / 工場 / HQ / 空港 / 港湾）
- 都市 / 工場 / HQ のHP回復量
- 補給ユニットの最大補給回数
- 自爆ドローン有効化
- 対ドローン迎撃確率 / 迎撃回数 / AIドローン生産比率上限
- 敵行動ログ表示
- 航空・海ユニット有効化
- 補給ユニットは `補給` コマンドで周囲1マスの味方へ燃料・弾薬を最大まで回復
- 補給車は地上ユニットのみ、空中補給機は航空ユニットのみ補給可能
- 補給回数は 1 回の実行ごとに 1 消費し、補給車は都市・工場・HQ、空中補給機は空港でターン終了すると全回復
- 燃料 / 弾薬消費有効化

...
describe('map preset rules', () => {
  it('ドローン戦マップID一覧は期待した4件に固定される', () => {
    expect(DRONE_FOCUSED_MAP_IDS).toEqual(['drone-factory-front', 'interceptor-belt', 'industrial-drone-raid', 'drone-sea-front']);
  });
  ...
  it('ドローン戦マップだけドローン戦プリセットを使う判定になる', () => {
    ...
    expect(isDroneFocusedMapId('drone-sea-front')).toBe(true);
